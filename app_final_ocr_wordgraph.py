...
if input_method == "Type Review Text":
    ...
    if uploaded_image is not None:
        st.image(uploaded_image, caption="Uploaded Review Screenshot", use_container_width=True)

        with st.spinner("Extracting text from image. This may take a moment the first time."):
        ...
        if extracted_text:
            review = st.text_area(
                "Review text extracted from image:",
                value=extracted_text,
                height=180
            )
        else:
            st.warning("No readable text was detected. Please upload a clearer screenshot or use the text input option.")
